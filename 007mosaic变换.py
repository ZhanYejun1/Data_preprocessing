...
def train_id(srcimage, saveBasePath):
    ...
    ftrain = open(os.path.join(saveBasePath, 'mosaic_id.txt'), 'w')  # 保存txt文件路径
...
def train_txt(srcimage, saveBasePath):
    ...
    for year, image_set in sets:
        image_ids = open(saveBasePath + 'mosaic_id.txt')
...
def save_mosaic_img(index):
    with open("mosaic_format.txt") as f:
        lines = f.readlines()
    ...
    if not os.path.exists(srcimage+"\\mosaic"):
        os.mkdir(srcimage+"\\mosaic")
    img.save(srcimage+"\\mosaic\\" + str(index).zfill(5) + ".jpg")  # TODO 这里改一下保存mosaic变换后的图片路径


if __name__ == '__main__':

    # 用这种方法批量读取图片，顺序不会打乱
    srcimage = "C:\\Users\\A3\\Desktop\\18\\"  # TODO 读取图片文件路径
    saveBasePath = "C:\\Users\\A3\\Desktop\\数据预处理\\"  # TODO 保存txt文件路径
    train_id(srcimage, saveBasePath)
    train_txt(srcimage, saveBasePath)

    num = 1  # TODO mosaic生成的图片数量，小于等于总图片数量/4
    index_mosaic = 0  # TODO  mosaic生成图片起始id  eg 0000.jpg 大于原图片索引
    # 保存mosaic图像
    while (num):
